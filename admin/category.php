...
/*=========== INSERT SLIDER START ===========*/
	...
	{
		extract($_POST);
		$sliderCheck = $general_cls_call->select_query("name", CATEGORY, "WHERE name=:name", array(':name'=>$txtName), 1);
		if($sliderCheck!='')
		{
		...
		else
		{
			...
			$field = "name, page_url, created_date, status";
			$value = ":name, :page_url, :created_date, :status";
			$addExecute=array(
			...
			$general_cls_call->insert_query(CATEGORY, $field, $value, $addExecute);
			header("location:".ADMIN_SITE_URL."category.php");
		}
	}
/*=========== INSERT SLIDER END ===========*/
...
/*=========== EDIT SLIDER START ===========*/
	...
	{	
		extract($_POST);
		$sliderCheck = $general_cls_call->select_query("name", CATEGORY, "WHERE name=:name AND id!=:id", array(':name'=>$txtName,'id'=>$_GET['LinkID']), 1);
		if($sliderCheck!='')
		{
			$erMsg = 'Name already exists.';
		}
		else
		{
			...
			$update_deal_city=$general_cls_call->update_query(CATEGORY, $setValues, $whereClause, $updateExecute);
			header("location:".ADMIN_SITE_URL."category.php");
		}
	}
/*=========== EDIT SLIDER END ===========*/
...
/*=========== SELECT QUERY START ===========*/
	$sliderVal = $general_cls_call->select_query("*", CATEGORY, "WHERE id=:id", array(':id'=>$_GET['LinkID']), 1);
/*=========== SELECT QUERY END ===========*/
...
/*=========== DELETE START ================*/
	...
	{
		$general_cls_call->delete_query(CATEGORY, "WHERE id=:id", array(':id'=>$_GET['id']));
		header("location:".ADMIN_SITE_URL."category.php");
	}
/*=========== DELETE END ================*/	
...
<!--
	function del(uid)
	{
		...
		{
		   document.frm.action='category.php?mode=del&id='+uid;
		   document.frm.submit();
		}
	}
//-->
      ...
      <article class="col-sm-12 col-md-12 col-lg-6" style="margin: 2% 0;">
        <div class="jarviswidget" id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false">
          <header> <span class="widget-icon"> <i class="fa fa-edit"></i> </span>
            <h2><?PHP echo ($_GET['LinkID']!='' ? 'Edit' : 'Add'); ?> Category</h2>
          </header>
          <div>
            ...
            <div>
              ...
              <div class="widget-body no-padding">
                <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                  <thead>
		<?PHP
			$sqlQuery = $general_cls_call->select_query("*", CATEGORY, "WHERE 1 ORDER BY id DESC", array(), 2);
			if($sqlQuery[0] != '')
			{
		?>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td>				  
						<?php	
							...
							{
						?>
                        <a href = "<?PHP echo ADMIN_SITE_URL; ?>category.php?staID=<?php echo($selectValue->id);?>&mode=0" title = "Click Inactive"><i class="fa fa-check-circle" style="color:green;"></i></a>
                        <?php
							}
							else
							{
						?>
                        <a href = "<?PHP echo ADMIN_SITE_URL; ?>category.php?staID=<?php echo($selectValue->id);?>&mode=1" title = "Click Active"><i class="fa fa-times-circle" style="color:red;"></i></a>
                        <?php
							}	
						?>&nbsp;<a href="<?PHP echo ADMIN_SITE_URL; ?>category.php?LinkID=<?php echo $selectValue->id; ?>" title="Click Edit"><i class="fa fa-pencil" style="color: #33a1ce;"></i></a>&nbsp;&nbsp;<a href="javascript: del('<?php echo $selectValue->id; ?>')" title="Click Delete"><i class="fa fa-trash-o" style="color:red;"></i></a>
					  </td>
                    </tr>
			<?PHP
					$i++;
